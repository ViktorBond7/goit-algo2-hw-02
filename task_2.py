...
def optimize_printing(print_jobs: List[PrintJob], constraints: PrinterConstraints) -> Dict:
    ...
    for job in sorted_jobs:
        ...
        # Перевіряємо чи можна додати в поточну партію
        if (
            current_items < constraints.max_items
            ...
            current_volume = job.volume
            current_items = 1
            current_time = job.print_time

    # Додаємо останню партію
    if current_batch:
        batches.append(current_batch)
        total_time += current_time
